...
public class LevelBuilder : MonoBehaviour {
	...
	********************************************/
	void Update() {
		int theBeat = isBeat ();
		//float timeStamp = Time.time - tIni;
		/**
		int notePattern = 0;
		bool isHarmonyNote = false;
		float timeStamp = Time.time;
		int noteCatcherNum = 0;
		noteCatcherNum = Mathf.RoundToInt (Random.Range (0.0f, 1.0f));
		*/
		//Phrase Controller needs to know notePattern, and time stamp to build a note and note catcher side of screen
		//Eventually need to detect harmonies and link them through scoring
		if ((theBeat &(int)beatType.kick) != (int)beatType.None) {
			Debug.Log ("Kick Detected ");
		//	notePattern = Mathf.RoundToInt (Random.Range (0.5f, 3.4f));

		}
		if ((theBeat & (int)beatType.snare) != (int)beatType.None) {
			Debug.Log ("Snare Detected ");
		//	notePattern = Mathf.RoundToInt (Random.Range (3.5f, 5.4f));
		}
		...
		/**
		if (notePattern != 0) {
			Debug.Log ("\n" + timeStamp + "," + notePattern + "," + noteCatcherNum);
						if (!isHarmonyNote) {
								//Debug.FileOut("\ntimeStamp, notePattern, noteCatchernum")
								Debug.Log ("\n" + timeStamp + "," + notePattern + "," + noteCatcherNum);
						} else {
